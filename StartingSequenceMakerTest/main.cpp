...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < ssm.getNumStates(); i++)
	{
		...
		printf("%i, %i %i\n", startTimesOld_ms[i], startTimes_ms[i], delta_ms);
		if(delta_ms != 0)
		{
	...
	for (int i = 0; i < ssm.getNumStates(); i++)
	{
		printf("%s, %s\n", spkrStatesOld[i]BOOL2STR, spkrStates[i]BOOL2STR);
		if(spkrStatesOld[i] != spkrStates[i])
		{
	...
	for (int i = 0; i < ssm.getNumStates(); i++)
	{
		printf("%s, %s\n", fpStatesOld[i]BOOL2STR, fpStates[i]BOOL2STR);
		if(fpStatesOld[i] != fpStates[i])
		{
